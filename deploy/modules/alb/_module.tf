resource "aws_lb" "main" {
  #bridgecrew:skip=CKV2_AWS_20: "Ensure that ALB redirects HTTP requests into HTTPS ones" False positive because only HTTP is used in the environments without a certificate.
  name               = var.alb_name
  internal           = false
...
resource "aws_alb_listener" "http" {
  #bridgecrew:skip=CKV_AWS_2: "Ensure ALB protocol is HTTPS"  False positive because only HTTP is used in the environments without a certificate.
  #bridgecrew:skip=CKV_AWS_103: "Ensure that load balancer is using TLS 1.2" False positive because only HTTP is used in the environments without a certificate.
